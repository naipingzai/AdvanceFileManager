...
# ===== Download source =====
$UpstreamUrl = "https://github.com/davemorrissey/subsampling-scale-image-view.git"
$UpstreamTag = "v3.9.0"
$ModulePath = "library"
...
if (-not (Test-Path "$SrcBase\java")) {
    ...
    Get-ChildItem "$SrcBase\java" -Recurse -Filter "*.java" | ForEach-Object {
        $content = [System.IO.File]::ReadAllText($_.FullName)
        $content = $content.Replace("android.support.annotation.", "androidx.annotation.")
        $content = $content.Replace("android.support.media.ExifInterface", "androidx.exifinterface.media.ExifInterface")
        [System.IO.File]::WriteAllText($_.FullName, $content)
    }
    Write-Host "Download complete (migrated to AndroidX)." -ForegroundColor Green
}
