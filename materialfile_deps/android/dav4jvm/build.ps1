...
if (-not (Test-Path "$SrcDir\.git")) {
    Write-Host "Cloning dav4jvm source..." -ForegroundColor Cyan
    & git clone --depth 1 https://github.com/bitfireAT/dav4jvm.git $SrcDir
    if ($LASTEXITCODE -ne 0) { throw "git clone failed" }
    Write-Host "Clone complete." -ForegroundColor Green
    'rootProject.name = "dav4jvm"' | Set-Content "$SrcDir\settings.gradle.kts"
}
...
$jar = Get-ChildItem "$ScriptDir\src\build\libs\*.jar" -Exclude "*-sources*","*-javadoc*" | Select-Object -First 1
if ($jar) {
