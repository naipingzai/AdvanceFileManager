...
OUTPUT_DIR="$PROJECT_ROOT/prebuild/native/zstd"
SRC_BASE="$LIB_DIR/src"

# Download source if not present
if [ ! -d "$SRC_BASE" ] || [ -z "$(ls -A "$SRC_BASE" 2>/dev/null)" ]; then
    echo "Downloading zstd 1.5.6..."
    ARCHIVE="$(mktemp).tar.gz"
    curl -L --progress-bar -o "$ARCHIVE" "https://github.com/facebook/zstd/archive/refs/tags/v1.5.6.tar.gz"
    mkdir -p "$SRC_BASE"
...
mv "$OUTPUT_DIR/lib/libzstd.a" "$OUTPUT_DIR/lib/libmaterialfile_zstd.a"

echo "zstd OK -> $OUTPUT_DIR"
